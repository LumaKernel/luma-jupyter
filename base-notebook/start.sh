...
# the luma user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the luma user to the desired the user (NB_USER)
    if id luma &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" luma 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the luma user:"
            _log "- username: luma       -> ${NB_USER}"
            _log "- home dir: /home/luma -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the luma user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the luma home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "luma" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/luma to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/luma/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/luma to /home/${NB_USER}!"
                _log "Attempting to symlink /home/luma to /home/${NB_USER}..."
                if ln -s /home/luma "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/luma to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/luma/"* ]]; then
...
# user we started as.
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old luma user to nayvoj ($(id -u lumlumaid -g luma))"
            ...
            sed --expression="s/^luma:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/luma:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "luma" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/luma. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/luma ]]; then
        _log "WARNING: no write access to /home/luma. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
